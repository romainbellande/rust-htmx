    ...
    fn call(&mut self, request: Request<B>) -> Self::Future {
        ...
        if request.extensions().get::<OidcAccessToken>().is_some() {
            ...
            Box::pin(async move {
                ...
                oidcclient.client = oidcclient.client.set_redirect_uri(RedirectUrl::new(
                    oidcclient.application_base_url.clone().to_string(),
                )?);

                println!("test 4");

                // print login session and query
                // println!("login_session {:?}", login_session);
                // println!("query {:?}", query);
    ...
    fn call(&mut self, request: Request<B>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            if let Some(login_session) = &mut login_session {
                ...
                //print login session refresh token
...
pub fn strip_oidc_from_path(base_url: Uri, uri: &Uri) -> Result<Uri, MiddlewareError> {
    ...
    base_url.path_and_query = uri
        .path_and_query()
        .map(|path_and_query| {
            let query: String = path_and_query
                .query()
                .and_then(|uri| {
                    uri.split('&')
                        .filter(|x| {
